...
export async function POST(request: Request) {
  ...
  const { data: session } = await admin.from("practice_sessions").select("id, prep_context_id").eq("id", parsed.data.sessionId).maybeSingle();
  if (!session || !await getOwnedPrepContext(session.prep_context_id)) return NextResponse.json({ error: "Session not found." }, { status: 404 });
  await admin.from("session_questions").delete().eq("session_id", session.id).is("answered_at", null);

...
export async function POST(request: Request) {
  ...
  const randomizedMappings = shuffle(mappings);
  ...
  const uniqueLeafIds = Array.from(new Set(scheduledLeafIds));
  const { data: skills } = await admin.from("ontology_skills").select("id, name").in("id", uniqueLeafIds);
  ...
  const { data: items, error: itemsError } = await admin.from("session_questions").insert(prepared.map((item, index) => ({ session_id: session.id, question_id: item.question.id, ontology_leaf_id: scheduledLeafIds[index], sequence_number: index + 1 }))).select("id, sequence_number, ontology_leaf_id");
  ...
  return NextResponse.json({ ok: true, sessionId: session.id, startedAt: session.created_at, durationMinutes: parsed.data.durationMinutes, questionCount, questions });
}

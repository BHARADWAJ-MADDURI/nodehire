...
export default async function SettingsPage() {
  ...
  const { data: providerKeys } = await createSupabaseAdminClient().from("user_provider_keys").select("provider").eq("user_id", data.user.id);
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-5 py-8 sm:px-8">
      ...
      <Card className="mt-6"><CardHeader><CardTitle>AI providers</CardTitle><CardDescription>Connect your own quota for live generation and evaluation beyond the shared cache.</CardDescription></CardHeader><CardContent><ProviderConnect authenticated initialConnected={(providerKeys ?? []).map((item) => item.provider)} /></CardContent></Card>

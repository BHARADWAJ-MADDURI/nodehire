...
export function MockInterviewClient({ prepContextId, company, role, authenticated }: { prepContextId: string; company: string; role: string; authenticated: boolean }) {
  ...
  return <main className="space-y-6"><div className="flex items-center justify-between"><div><p className="text-sm text-primary">Question {index + 1} of {questions.length}</p><h1 className="mt-1 text-2xl font-semibold">Mock interview</h1></div><div className="flex gap-2"><Badge>{current.answer_type}</Badge>{speech.isListening && <Badge className="animate-pulse">Listening — auto-stop</Badge>}</div></div><div className="h-2 rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all" style={{ width: `${((index + 1) / questions.length) * 100}%` }}/></div><Card className="glass-panel border-0"><CardHeader><CardTitle className="text-xl leading-8">{current.question_text}</CardTitle><CardDescription>{current.runtimeContext.framing}</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex gap-2">{speech.speechSupported && <Button variant="outline" size="sm" onClick={() => speech.speak(current.question_text)}><Volume2/>Speak question naturally</Button>}{current.answer_type === "text" && speech.browserSupported && (speech.isListening ? <Button variant="destructive" size="sm" onClick={speech.stopListening}><MicOff/>Stop now</Button> : <Button variant="outline" size="sm" onClick={() => speech.startListening(answer)}><Mic/>Answer with voice</Button>)}</div>{current.answer_type === "text" && !speech.browserSupported && <p className="rounded-xl border border-amber-500/25 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-300">Voice input is unavailable in this browser. Text input always remains available; try current Chrome or Edge.</p>}{speech.error && <p className="text-sm text-destructive">{speech.error}</p>}{current.answer_type === "diagram" ? <DiagramCanvas key={index} ref={diagramRef}/> : <Textarea value={answer} onChange={(event) => setAnswer(event.target.value)} className={`min-h-64 ${current.answer_type === "code" ? "font-mono text-sm" : ""}`} placeholder="Give your interview answer…"/>}{evaluation ? <div className="rounded-xl border bg-primary/5 p-4 text-sm"><strong>{evaluation.score}/100.</strong> {evaluation.feedback}</div> : !needProvider && <Button onClick={submit} disabled={busy}>{busy ? <Loader2 className="animate-spin"/> : "Submit answer"}</Button>}{needProvider && <div className="rounded-xl border bg-background/70 p-4"><h3 className="mb-1 font-semibold">Optional: connect your API key for AI feedback</h3><p className="mb-3 text-xs text-muted-foreground">Your provider&apos;s pricing, free tier, and usage limits apply.</p><ProviderConnect authenticated={authenticated} compact /></div>}{(evaluation || report.length === index + 1) && <Button onClick={next} disabled={index >= questions.length - 1}>Next question</Button>}{error && <p className="text-sm text-destructive">{error}</p>}</CardContent></Card></main>;
}

...
export default async function DrillPage({ params, searchParams }: { params: Promise<{ contextId: string }>; searchParams: Promise<{ leaf?: string }> }) {
  ...
  return <DrillClient prepContextId={context.id} company={context.company} role={context.role} topics={skills ?? []} authenticated={Boolean(context.user_id)} preselectedLeafId={leaf} />;
}

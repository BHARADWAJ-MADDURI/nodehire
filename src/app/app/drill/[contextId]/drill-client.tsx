...
export function DrillClient({ prepContextId, company, role, topics, authenticated, preselectedLeafId }: { prepContextId: string; company: string; role: string; topics: Topic[]; authenticated: boolean; preselectedLeafId?: string }) {
  ...
  async function nextQuestion() {
    ...
    if (topicQuestionNumber < QUESTIONS_PER_TOPIC) {
      await startForTopic(topicId, topicQuestionNumber + 1);
      return;
    }
    const currentIndex = Math.max(0, topics.findIndex((topic) => topic.id === topicId));
    await startForTopic(topics[(currentIndex + 1) % topics.length]?.id ?? topicId, 1);
  }
  ...
  return <main className="space-y-6">
    ...
    {active && <div className="flex items-center justify-between rounded-xl border bg-card/50 px-4 py-3 text-sm"><span><strong>{topics.find((topic) => topic.id === topicId)?.name}</strong> deep dive</span><span className="text-muted-foreground">Question {topicQuestionNumber} of {QUESTIONS_PER_TOPIC} in this topic</span></div>}

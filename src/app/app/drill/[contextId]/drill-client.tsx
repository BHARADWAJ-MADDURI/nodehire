...
export function DrillClient({ prepContextId, company, role, topics, authenticated, preselectedLeafId }: { prepContextId: string; company: string; role: string; topics: Topic[]; authenticated: boolean; preselectedLeafId?: string }) {
  ...
  return <main className="space-y-6">
    ...
    {selfGrade && <Card className="border-amber-500/25 bg-amber-500/5"><CardHeader><CardTitle>Self-grade this turn</CardTitle><CardDescription>Live AI evaluation is unavailable. Your attempt was not scored.</CardDescription></CardHeader><CardContent className="space-y-5"><p className="text-sm leading-6 text-muted-foreground">{selfGrade}</p>{needProvider && <div className="rounded-xl border bg-background/70 p-4"><h3 className="mb-3 font-semibold">Keep going — connect a free key</h3><ProviderConnect authenticated={authenticated} compact /></div>}<div className="flex gap-2">{needProvider && <Button onClick={submit}>Evaluate after connecting</Button>}<Button variant="outline" onClick={nextQuestion}>Choose next question</Button></div></CardContent></Card>}

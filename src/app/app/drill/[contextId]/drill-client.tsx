...
export function DrillClient({ prepContextId, company, role, topics }: { prepContextId: string; company: string; role: string; topics: Topic[] }) {
  ...
  return <main className="space-y-6">
    ...
    {active && <Card className="glass-panel border-0"><CardHeader><div className="flex flex-wrap gap-2"><Badge>{difficulty}</Badge><Badge variant="outline">{active.cacheHit ? "Cache hit" : "Cache seeded"}</Badge>{speech.isListening && <Badge className="animate-pulse">Listening live</Badge>}</div><CardTitle className="mt-4 text-xl leading-8">{active.questionText}</CardTitle><CardDescription>{active.framing}</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-2">{speech.speechSupported && <Button type="button" variant="outline" size="sm" onClick={() => speech.speak(active.questionText)}><Volume2 />Speak question</Button>}{speech.browserSupported && (speech.isListening ? <Button type="button" variant="destructive" size="sm" onClick={speech.stopListening}><MicOff />Stop listening</Button> : <Button type="button" variant="outline" size="sm" onClick={() => speech.startListening(answer)}><Mic />Answer with voice</Button>)}</div>{!speech.browserSupported && <p role="status" className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-300">Voice input is not supported in this browser. Text input remains fully available; try current Chrome or Edge for microphone dictation.</p>}{speech.error && <p role="alert" className="text-sm text-destructive">{speech.error}</p>}<Textarea value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder="Speak or type your answer. You can edit the transcript before submitting…" className="min-h-56" /><p className="text-xs text-muted-foreground">Voice transcription stays editable. Stop listening before reviewing and submitting.</p><Button onClick={submit} disabled={busy || !answer.trim()}>{busy ? <Loader2 className="animate-spin" /> : "Submit answer"}</Button></CardContent></Card>}

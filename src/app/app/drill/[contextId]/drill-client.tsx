...
import { ArrowRight, CheckCircle2, Loader2, Mic, MicOff, Volume2 } from "lucide-react";
...
export function DrillClient({ prepContextId, company, role, topics }: { prepContextId: string; company: string; role: string; topics: Topic[] }) {
  ...
  async function start() {
    setBusy(true); setError(""); setEvaluation(null); setAnswer(""); setSubmittedAnswer(""); setIdealAnswer(""); setPendingFollowUp(null); setSelfGrade(null);
  ...
  async function submit() {
    ...
    const response = await fetch("/api/evaluate", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: active.sessionId, sessionQuestionId: active.sessionQuestionId, answer, answerImage }) });
    const result = await response.json();
    if (response.ok) {
  ...
  async function nextQuestion() {
    ...
    setActive(null); setAnswer(""); setEvaluation(null); setSubmittedAnswer(""); setIdealAnswer(""); setPendingFollowUp(null); setSelfGrade(null); setError("");
  }
  ...
  return <main className="space-y-6">
    ...
    {active && <Card className="glass-panel border-0"><CardHeader><div className="flex flex-wrap gap-2"><Badge>{difficulty}</Badge><Badge variant="outline">{active.cacheHit ? "Cache hit" : "Cache seeded"}</Badge><Badge variant="outline">{active.answerType}</Badge>{speech.isListening && <Badge className="animate-pulse">Listening — auto-stops after your pause</Badge>}</div><CardTitle className="mt-4 text-xl leading-8">{active.questionText}</CardTitle><CardDescription>{active.framing}</CardDescription></CardHeader><CardContent className="space-y-4"><div className="flex flex-wrap gap-2">{speech.speechSupported && <Button type="button" variant="outline" size="sm" onClick={() => speech.speak(active.questionText)}><Volume2 />Speak question naturally</Button>}{active.answerType === "text" && speech.browserSupported && (speech.isListening ? <Button type="button" variant="destructive" size="sm" onClick={speech.stopListening}><MicOff />Stop now</Button> : <Button type="button" variant="outline" size="sm" onClick={() => speech.startListening(answer)}><Mic />Answer with voice</Button>)}</div>{active.answerType === "text" && !speech.browserSupported && <p role="status" className="rounded-xl border border-amber-500/25 bg-amber-500/10 px-4 py-3 text-sm text-amber-700 dark:text-amber-300">Voice input is not supported in this browser. Text input remains fully available; try current Chrome or Edge for microphone dictation.</p>}{speech.error && <p role="alert" className="text-sm text-destructive">{speech.error}</p>}{active.answerType === "diagram" ? <DiagramCanvas ref={diagramRef} /> : <><Textarea value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder={active.answerType === "code" ? "Write your solution and explain complexity and edge cases…" : "Speak or type your answer. You can edit the transcript before submitting…"} className={`min-h-56 ${active.answerType === "code" ? "font-mono text-sm" : ""}`} /><p className="text-xs text-muted-foreground">{active.answerType === "code" ? "AI-assessed, not run against test cases. Complexity is estimated, not verified." : "Voice transcription auto-stops after a short pause and remains editable before submission."}</p></>}<Button onClick={submit} disabled={busy || Boolean(evaluation) || (active.answerType !== "diagram" && !answer.trim())}>{busy ? <Loader2 className="animate-spin" /> : evaluation ? "Answer submitted" : "Submit answer"}</Button></CardContent></Card>}
    {evaluation && <Card className="border-primary/25 bg-primary/5"><CardHeader><CardTitle className="flex items-center justify-between"><span className="flex items-center gap-2"><CheckCircle2 className="text-primary" />Evaluation</span><span>{evaluation.score}/100</span></CardTitle><CardDescription>{evaluation.feedback}</CardDescription></CardHeader><CardContent className="space-y-6"><div className="grid gap-5 sm:grid-cols-2"><div><h3 className="text-sm font-semibold">Strengths</h3>{evaluation.strengths.length ? <ul className="mt-2 space-y-1 text-sm text-muted-foreground">{evaluation.strengths.map((item) => <li key={item}>✓ {item}</li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">No demonstrated strengths in this response yet.</p>}</div><div><h3 className="text-sm font-semibold">Improve next</h3><ul className="mt-2 space-y-1 text-sm text-muted-foreground">{evaluation.gaps.map((item) => <li key={item}>• {item}</li>)}</ul></div></div><div className="rounded-xl border bg-background/60 p-4"><h3 className="text-sm font-semibold">Your submitted answer</h3><p className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">{submittedAnswer}</p></div><div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4"><h3 className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">Model interview answer</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{idealAnswer || evaluation.betterInterviewAnswer}</p></div><div className="flex flex-wrap gap-2">{pendingFollowUp && <Button onClick={tryFollowUp}>Try guided follow-up</Button>}<Button variant="outline" onClick={nextQuestion}>Choose next question</Button></div></CardContent></Card>}
    {selfGrade && <Card className="border-amber-500/25 bg-amber-500/5"><CardHeader><CardTitle>Self-grade this turn</CardTitle><CardDescription>Live vision evaluation is unavailable. Your diagram was not scored.</CardDescription></CardHeader><CardContent className="space-y-4"><p className="text-sm leading-6 text-muted-foreground">{selfGrade}</p><Button variant="outline" onClick={nextQuestion}>Choose next question</Button></CardContent></Card>}

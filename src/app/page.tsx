...
export default function Home() {
  ...
  async function startDemo() {
    ...
    try {
      ...
      setPending(false);
      setError("Demo mode is waking up. Please try again in a moment.");
    }
  }
    ...
    <main className="min-h-screen overflow-hidden">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-8">
        ...
        <nav className="flex items-center gap-1" aria-label="Primary navigation">
          ...
          <Button onClick={startDemo} disabled={pending}>Try demo</Button>
        </nav>
      </header>
      <section className="mx-auto grid max-w-7xl gap-14 px-5 pb-24 pt-16 sm:px-8 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:pt-24">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Button size="lg" onClick={startDemo} disabled={pending} className="h-12">{pending ? "Starting…" : "Try the 30-day demo"}<ArrowRight /></Button><Link href="/login" className={buttonVariants({ variant: "outline", size: "lg", className: "h-12" })}>Google or magic link</Link></div>

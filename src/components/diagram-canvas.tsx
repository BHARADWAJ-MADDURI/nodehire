...
import { ArrowUpRight, Box, Eraser, Link2, Pencil, Plus, Type, Undo2 } from "lucide-react";
...
export const DiagramCanvas = forwardRef<DiagramCanvasRef>(function DiagramCanvas(_, ref) {
  ...
  function addKeyboardBox() {
    const boxCount = elements.filter((element) => element.kind === "box").length;
    setElements((items) => [...items, { kind: "box", x: 50 + (boxCount % 4) * 200, y: 60 + Math.floor(boxCount / 4) * 130, width: 160, height: 70, label }]);
  }
  function connectLastBoxes() {
    const boxes = elements.filter((element): element is Extract<Element, { kind: "box" }> => element.kind === "box");
    if (boxes.length < 2) return;
    const [from, to] = boxes.slice(-2);
    setElements((items) => [...items, { kind: "arrow", from: { x: from.x + from.width, y: from.y + from.height / 2 }, to: { x: to.x, y: to.y + to.height / 2 } }]);
  }
  ...
  return <div className="space-y-3"><div className="flex flex-wrap items-center gap-2">{([['box', Box, 'Box'], ['arrow', ArrowUpRight, 'Arrow'], ['draw', Pencil, 'Draw'], ['text', Type, 'Text']] as const).map(([value, Icon, name]) => <Button key={value} type="button" size="sm" variant={tool === value ? "default" : "outline"} onClick={() => setTool(value)}><Icon />{name}</Button>)}<Input value={label} onChange={(event) => setLabel(event.target.value)} aria-label="Shape label" className="h-9 w-36" /><Button type="button" size="sm" variant="outline" onClick={addKeyboardBox}><Plus />Add labeled box</Button><Button type="button" size="sm" variant="outline" onClick={connectLastBoxes} disabled={elements.filter((element) => element.kind === "box").length < 2}><Link2 />Connect last two</Button><Button type="button" size="icon-sm" variant="ghost" aria-label="Undo" onClick={() => setElements((items) => items.slice(0, -1))}><Undo2 /></Button><Button type="button" size="icon-sm" variant="ghost" aria-label="Clear diagram" onClick={() => setElements([])}><Eraser /></Button></div><svg ref={svgRef} role="img" aria-label={`System design diagram with ${elements.length} elements`} viewBox="0 0 900 520" onPointerDown={down} onPointerMove={move} onPointerUp={up} className="aspect-[900/520] w-full touch-none rounded-xl border bg-white text-slate-900 shadow-inner"><defs><marker id="arrowhead" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><polygon points="0 0, 10 3.5, 0 7" fill="#334155" /></marker></defs>{elements.map((element, index) => element.kind === "box" ? <g key={index}><rect x={element.x} y={element.y} width={element.width} height={element.height} rx="10" fill="#eff6ff" stroke="#4f46e5" strokeWidth="3"/><text x={element.x + element.width / 2} y={element.y + element.height / 2} dominantBaseline="middle" textAnchor="middle" fontSize="17" fill="#1e293b">{element.label}</text></g> : element.kind === "arrow" ? <line key={index} x1={element.from.x} y1={element.from.y} x2={element.to.x} y2={element.to.y} stroke="#334155" strokeWidth="3" markerEnd="url(#arrowhead)" /> : element.kind === "text" ? <text key={index} x={element.x} y={element.y} fontSize="18" fill="#1e293b">{element.label}</text> : <polyline key={index} points={element.points.map((item) => `${item.x},${item.y}`).join(" ")} fill="none" stroke="#334155" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />)}</svg><p className="text-xs text-muted-foreground">Drag to place boxes and arrows, or use “Add labeled box” and “Connect last two” with a keyboard. The diagram stays in this browser until submission.</p></div>;
});
